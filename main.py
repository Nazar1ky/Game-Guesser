...
while True:
    ...
    data = requests.get(
        f"https://api.steampowered.com/IStoreService/GetAppList/v1/?include_games=true&max_results=50000&last_appid={offset}&key={api_key}",
        timeout=10,
    ).json()
    ...
    for product in products:
        ...
        if search_type == 1:
            pattern = (
                "^" + re.sub(r"\*", ".", search_input) + "$"
            )  # Replace all * for . | Because . is any symbol | Add ^ Because that start of line | Add $ because that end of line | https://regex101.com/r/FsuHFN/1
        elif search_type == 2:
            ...
            for i in range(len(product_name_words)):
                if len(product_name_words) != len(
                    starts_with,
                ) or not product_name_words[i].lower().startswith(starts_with[i]):
                    end = True
                    break
